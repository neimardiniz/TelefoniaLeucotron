...
$sqlultimachamada="SELECT
p.cham_identificacao,
p.cham_hora,
p.cham_duracaoligacao

from usuarios_chamadas p, usuarios pp,contatos ppp
where
p.cham_hora = (
        SELECT 
            MAX(p.cham_hora )  from usuarios_chamadas p, usuarios pp,contatos ppp
            where p.cham_usuario_id = pp.usuario_id
and ppp.contato_id = pp.usu_contato_id

and p.cham_data = '$dataatual'
and ppp.con_nome='$usuario'

            )

and p.cham_usuario_id = pp.usuario_id
and ppp.contato_id = pp.usu_contato_id
...
$tempochamada=0;
  ...
  echo "Sua ultima ligacao gravada: <br> Numero: $numerochamada / Hora: $ultimachamada / Tempo: $tempochamada";
